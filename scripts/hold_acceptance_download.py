"""Windows-only: hold one synthetic offline copy open to reproduce delete failure.
...
def main():
    ...
    path = home / 'offline-reading' / 'downloads' / (args.job_id + '.json')
    if (os.name != 'nt' or not home.name.startswith('rynmesh-offline-acceptance-')
            or not re.fullmatch('[a-f0-9]{32}', args.job_id)
            or path.resolve() != path or not path.is_file()):
        raise SystemExit('Use one existing synthetic offline acceptance download on Windows.')
